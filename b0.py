...
def print_header(text):
    ...
    print(f"{C.CYAN}║{C.RESET} {C.MAGENTA}{C.BOLD}🚀 RUIJIE BYPASS PRO v2.1{C.RESET}{' ' * (58 - len('🚀 RUIJIE BYPASS PRO v2.1') - 2)} {C.CYAN}║{C.RESET}")
    print(f"{C.CYAN}║{C.RESET} {C.WHITE}🔧 )Internet Freedom Tool{C.RESET}{' ' * (58 - len('🔧 Galaxy-Grade Internet Freedom Tool') - 2)} {C.CYAN}║{C.RESET}")
    print(f"{C.CYAN}╚{'═' * 58}╝{C.RESET}")
    print(f"{C.DIM}{' ' * 10}⚡ Owner: @kuranomi10⚡{C.RESET}")
    print_separator()
...
def check_license(user_key):
    """Check license against GitHub-hosted key file"""
    dev_id = get_device_id()
    net_time = get_network_time()
    curr_time = datetime.now()
    
    # Anti-tamper: check time travel
    ...
    try:
        # Fetch key database from GitHub
...
│ {C.WHITE}📡 Contact{C.RESET}      : {C.MAGENTA}@kuranomi10{C.RESET}{' ' * (50 - len('@SKB4402') - 20)}│
{C.CYAN}└────────────────────────────────────────────────────┘{C.RESET}
""")
...
│ {C.CYAN}4{C.RESET} 📊  Show System Status        │
...
def key_activation_flow():
    ...
    print(f"├──────────────────────────────────────┤")
    print(f"│  Contact {C.MAGENTA}@kuranomi10{C.RESET} to get key │")
    print(f"└──────────────────────────────────────┘{C.RESET}")
